    ...
    m_height(height)
{
    ...
    m_allShapes = std::vector<std::shared_ptr<OpenGLShape>>();
    m_allShapes.push_back(std::make_shared<Cube>(settings.shapeParameter1, settings.shapeParameter2));
    m_allShapes.push_back(std::make_shared<Cone>(settings.shapeParameter1, settings.shapeParameter2));
    m_allShapes.push_back(std::make_shared<Sphere>(settings.shapeParameter1, settings.shapeParameter2));
    m_allShapes.push_back(std::make_shared<Cylinder>(settings.shapeParameter1, settings.shapeParameter2));

    m_lastShape = SHAPE_CUBE;
    m_shape = m_allShapes[m_lastShape];
}
...
//    m_shape = std::make_unique<Sphere>(settings.shapeParameter1, settings.shapeParameter2);
    ...
    m_lastShape = settings.shapeType;
    if(m_lastShape > SHAPE_CYLINDER){
        std::cout << "The selected shape has not been implemented. Please make a new selection" << std::endl;
    } else {
        m_shape = m_allShapes[m_lastShape];
    }
